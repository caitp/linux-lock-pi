...
module.exports.GPIO = function() {
    ...
    if(typeof config === "object") {
      ...
      async.each(config, function(sub, done) {
        ...
        async.eachSeries(action, function(act, done) {
          ...
          //
          var p = wiringToNative(act.pin || pin)
          ...
          async.every(verbs, function(verb, done) {
            if(typeof act[verb] === "number") {
              ...
              execVerb[verb](wiringToNative(p), act[verb], done)
            } else done()
          }, function(err) {
            //
            // Finished with verbs
  ...
  //
  // GPIO#wiringToNative(pin)
  //
  // Private API, convert a wiringPi pin number into a Raspberry Pi header number,
  // or return undefined.
  //
  function wiringToNative(pin) {
    return pinMap[pin]
  }

  //
  // Get each GPIO pin ready for use before considering the object actually finished.
  //
  async.each(Object.keys(pinMap), function(pin, done) {
    this.gpioPins[pin] = gpio.export(pinMap[pin], {
        ready: function() { done() }
    }, function() {
  ...
  // GPIO#shutdown()
  //
  // Allows each pin to be 'unexported', and presumably causes the OS to release
  // file handles. Since there is no way for me to "force" this to be called, I
  // suggest extreme caution.
  //
  // Also, I suggest not using any heavy machine motors or other heavy moving
  // parts, as there is absolutely no safety guarantee whatsoever. Fires could
  // happen, limbs could be severed, you have been warned.
  //
