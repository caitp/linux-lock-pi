...
//
//
//   Returns: Object (node-restify JsonClient). This Restify client is a customized
//            instance featuring LinuxLock-specific routines:
//            JsonClient#isRFIDAuthorized(rfidNo)
//
//   Description: Create an instance of a JsonClient, which will interact with the
//   LinuxLock JSON web service / API.
//
module.exports.createClient = function(options) {
  ...
  // Bind APIs
  client.checkRFID = checkRFID.bind(client)
...
//             encoded and passed to the LinuxLock JSON Web Service, in order to
//             determine whether or not the card is authorized.
//
//   Description: Sends a request to the web service, which shall in turn emit
//   either an 'authorized' event, or an 'unauthorized' event. The event data for
//   these events shall contain the deserialized RFID tag under 'tag', and the
//   type 'rfid', EG "{type: 'rfid', tag: '01AB9607AD'}"
//
//   This routine is not exported, but is bound to the JsonClient on creation.
//   As such, 'this' should always be valid.
//
function checkRFID(rfidNo) {
  this.get('/api/auth/rfid/'+encodeURIComponent(rfidNo)+'.json',
  function(err,req,res,obj) {
    if(err && (!obj || !"auth" in obj)) {
      console.log("Unexpected response: " + res + "("+err+")")
      obj = {auth: false}
    }
    var eventArg = { type: 'rfid', tag: rfidNo }
    if(obj.auth) this.emit('authorized', eventArg)
    else this.emit('deauthorized', eventArg)
  })
}
